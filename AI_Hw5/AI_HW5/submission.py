...
class ExactInference(object):
    ...
    """
    explanation : 
    for every available tile, caculate the distance between the tile and the car agent as mean value.
    This mean value along with the standard deviation and the observed distance value are passed into PDF function.
    Then the pdf output is set to be emission probability.
    Finally, update the belief value of the tile with the original belief multiplied by the emission probability,
    then normalize the calculated belief value.
    """
    def observe(self, agentX: int, agentY: int, observedDist: float) -> None:
        # BEGIN_YOUR_CODE (our solution is 9 lines of code, but don't worry if you deviate from this)
        for row in range(self.belief.numRows) :
            for col in range(self.belief.numCols):
                # mean = true distance from tile to my car
                mean_dist = math.sqrt( (agentX - util.colToX(col))**2 + (agentY - util.rowToY(row))**2 )
                # emission probability is determined by PDF
                emission = util.pdf(mean_dist, Const.SONAR_STD, observedDist)
                # update belief : emission * belief
                self.belief.setProb(row,col, emission * self.belief.getProb(row, col))
        self.belief.normalize() # normalize belief
        # END_YOUR_CODE
    ...
    """
    explanation :
    First create a new belief dictionary to store the updated values.
    For every possible new tile that is described in the transition table,
    the value in the index of the new tile is updated with the total sum of related transition probability
    multiplied by original belief value.
    Then, assign the original belief table to normalized new one.  
    """
    def elapseTime(self) -> None:
        ...
        # create new belief grid with all value 0
        new_belief = util.Belief(self.belief.numRows, self.belief.numCols, 0) 

        for oldTile, newTile in self.transProb:
            # each belief = sigma (transition * belief)
            new_belief.addProb(newTile[0], newTile[1], self.transProb[(oldTile,newTile)] * 
                                self.belief.getProb(oldTile[0],oldTile[1])) 
...
class ParticleFilter(object):
    ...
    """
    explanation :
    create a dictionary to store re-weighted particles' locations.
    For every possible partiacle location, calculate the mean value and emission probability
    following the same step in part one. Then update the new particles distribution with the 
    emission probability with the observed distance.
    Create another dictionary to store re-sampled particles' locations.
    For each one of #num_particles particles, use the weighted random distribution to determine
    the location to store the distibuted particle. The number of the particle placed in the location
    is incremented by one. Finally update the dictionary and belief value.
    """
    def observe(self, agentX: int, agentY: int, observedDist: float) -> None:
        # BEGIN_YOUR_CODE (our solution is 12 lines of code, but don't worry if you deviate from this)
        new_particle = collections.defaultdict(int) 
        # re-weight each particle
        for row,col in self.particles:
            mean_dist = math.sqrt( (agentX - util.colToX(col))**2 + (agentY - util.rowToY(row))**2 ) # mean value
            emission = util.pdf(mean_dist, Const.SONAR_STD, observedDist)
            new_particle[(row,col)] = emission * self.particles[(row,col)]
        # re-sampling |self.NUM_PARTICLES| new particles using weightedRandomChoice()
        res_new_particle = collections.defaultdict(int)
        for _ in range(self.NUM_PARTICLES):
            # add a randomly chosen particle to dict
            random_particle = util.weightedRandomChoice(new_particle) 
            res_new_particle[random_particle] += 1
        self.particles = res_new_particle
        # END_YOUR_CODE
        self.updateBelief()
    ...
    """
    explanation :
    Create a new dictionary for transitted particles.
    For each particle location, if it's in the transition table, 
    use the weighted random function to select a distributed place to store a new particle for each particle
    , adding the particle number at that location by one. Finally update the table.
    """
    def elapseTime(self) -> None:
        ...
        for particle in self.particles:
            if particle in self.transProbDict: # make sure the particle is in transition
                new_weight_p = self.transProbDict[particle]
                for p in range(self.particles[particle]):
                    # re-sample for each new particle at particular location
                    random_p = util.weightedRandomChoice(new_weight_p)
                    new_particle[random_p] += 1
