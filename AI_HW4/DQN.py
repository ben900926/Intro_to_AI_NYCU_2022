...
class Agent():
    ...
    def learn(self):
        ...
        next_states_batch = torch.tensor(next_state,dtype=torch.float32)
        
        # compute Q values
        q_eval = self.evaluate_net.forward(state_batch)

        # compute target Q
        q_target = self.target_net.forward(next_states_batch)
        loss = torch.tensor(0.,dtype=torch.float32)
        for q,a,r,qt,d in zip(q_eval,action,reward_batch,q_target,done):
            q = q[a] # Q(s,a)
            target = r + self.gamma * (1-d) * torch.max(qt)
            # compute MSE loss : (y - Q(s,a))^2
            loss += (q - target)**2 / self.batch_size
        
        # optimize the model
        self.optimizer.zero_grad() # zero-out the grad
        loss.backward()
    ...
    def choose_action(self, state):
        ...
        with torch.no_grad():
            ...
            else: # follow policy net
                state_batch = torch.as_tensor(state,dtype=torch.float32)
                return int(torch.argmax(self.evaluate_net.forward(state_batch),dim=0))
            # End your code
    ...
    def check_max_Q(self):
        ...
        """
        # Begin your code
        return torch.max(self.evaluate_net.forward(torch.tensor(env.reset(),dtype=torch.float32)))
        # End your code
