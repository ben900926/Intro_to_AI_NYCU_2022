...
class Agent():
    ...
    def discretize_observation(self, observation):
        ...
        # Begin your code
        discrete_vals = []
        # create discretized value for each parameter
        for i in range(len(observation)):
            discrete_vals.append(self.discretize_value(observation[i],self.bins[i])) # turn bin to index
    ...
    def choose_action(self, state):
        ...
        # explore a random action if explore rate > random number
        if self.epsilon > random_num:
            return np.random.randint(env.action_space.n)
        # exploit an action (follows the policy) with largest q value if not
        else:
    ...
    def learn(self, state, action, reward, next_state, done):
        ...
        # Begin your code
        if not done:
            # optimal q value = q + learning_rate*(reward + discount_rate*max_expected_qValue-q)
            max_next_q = np.max(self.qtable[tuple(next_state)])
            q = self.qtable[tuple(state)][action] # insert the action tuple to the last of state tuple
            new_q = q + self.learning_rate*(reward + self.gamma* max_next_q - q)
            self.qtable[tuple(state)][action] = new_q
        else:
            q = self.qtable[tuple(state)][action] # insert the action tuple to the last of state tuple
            new_q = q + self.learning_rate*(reward - q)
            self.qtable[tuple(state)][action] = new_q
        # End your code
    ...
    def check_max_Q(self):
        ...
        """
        # Begin your code
        state = self.discretize_observation(env.reset()) # initial discretized state
